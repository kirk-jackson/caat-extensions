...
class CRM_Contribute_Form_AbstractEditPayment extends CRM_Contact_Form_Task {
  ...
  public function buildValuesAndAssignOnline_Note_Type($id, &$values) {
    ...
    if (!$this->_online) {
      ...
      // VEDA: Make additional Details editable if payment processor supports backoffice edit
      if ($this->_online) {
        $paymentProcessorId = CRM_Core_DAO::getFieldValue('CRM_Contribute_DAO_ContributionRecur', $this->_online, 'payment_processor_id');
        $backOfficePaymentProcessors = CRM_Financial_BAO_PaymentProcessor::getPaymentProcessors(array('BackOffice'), array($paymentProcessorId));
