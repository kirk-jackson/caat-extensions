 ...
 */
function offlinerecurringcontributions_civicrm_tabset($tabsetName, &$tabs, $context) {
  ...
  if (array_key_exists('CiviContribute', $components)
    && !empty($viewOptions['CiviContributeRecur'])
    && CRM_Core_Permission::access('CiviContribute')) {

    // Find position of Contribution tab
    // So that we can add Recurring Contribution tab next to ot
    foreach($tabs as $position => $tab) {
      if ($tab['id'] == 'contribute') {
        $contributePosition = $position + 1;
      }
    }

    // Recurring Contribution tab
    $recurTab[$contributePosition] = array(
      'id' => 'contribute-recur',
      'url' => CRM_Utils_System::url("civicrm/contact/view/contributionrecurtab", "reset=1&force=1&cid={$context['contact_id']}"),
      'title' => ts('Recurring Contributions'),
      'weight' => 21, 
      'count' => CRM_Contribute_BAO_ContributionRecur::contributionRecurCount($context['contact_id']),
      'class' => 'livePage',
    );

    //Insert Recurring Contribution tab next to Contribution tab
    $tabs = array_merge(
      array_slice($tabs, 0, $contributePosition),
      $recurTab,
      array_slice($tabs, $contributePosition)
    );
  }
}
